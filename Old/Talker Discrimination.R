...
for(d in 1:length(date)){
  ...
  for(f in 1:length(files)){
    ...
    write.xlsx(Data2, paste0(participant[p],"_",calDate,"_Talker_Discrimination_",task[f],"_Scored.xlsx"),showNA = F)
    ...
    if(move_to_analysis == T){
      ...
      files2 <- files2[!grepl("BA_Scored", files2)]
      files2 <- files2[!grepl("CI_Scored", files2)]
